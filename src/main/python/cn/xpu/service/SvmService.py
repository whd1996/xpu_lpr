...
import cn.xpu.service.ImgRecognitionService as img_recognition
...
class CardPredictor:
    ...
    def train_svm(self):
        ...
        if os.path.exists("src/main/resource/data/svm.dat"):
            self.model.load("src/main/resource/data/svm.dat")
        if os.path.exists("src/main/resource/data/svmchinese.dat"):
            self.modelchinese.load("src/main/resource/data/svmchinese.dat")
    ...
    def img_first_pre(self, car_pic_file):
        ...
        oldimg = img
        cv2.imwrite("src/main/tmp/oldimg.jpg", oldimg)
        ...
        cv2.imwrite("src/main/tmp/img_gray.jpg", img)
        ...
        cv2.imwrite("src/main/tmp/img_opening.jpg", img_opening)
        # 创建20*20的元素为1的矩阵 开操作，并和img重合

        # Otsu’s二值化
        ret, img_thresh = cv2.threshold(img_opening, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        cv2.imwrite("src/main/tmp/img_threshold.jpg", img_thresh)

        # Canny 边缘检测
        # 较大的阈值2用于检测图像中明显的边缘  一般情况下检测的效果不会那么完美，边缘检测出来是断断续续的
        # 较小的阈值1用于将这些间断的边缘连接起来
        img_edge = cv2.Canny(img_thresh, 100, 200)
        cv2.imwrite("src/main/tmp/img_edge.jpg", img_edge)
        ...
        cv2.imwrite("src/main/tmp/img_xingtai.jpg", img_edge2)
        ...
        for i, color in enumerate(colors):
            ...
            if color in ("blue", "yellow", "green"):
                ...
                try:
                    ...
                    cv2.imwrite("src/main/tmp/gray_img.jpg", gray_img)
                except:
                    print("gray转换失败")
                ...
                cv2.imwrite("src/main/tmp/chepai_xingtai.jpg", gray_img)
                ...
                card_color = color
                break
        cv2.imwrite("src/main/tmp/img_caijian.jpg", roi)
